...
for run in ${pruns[@]}; do
  log_echo "...run ${run}"
  log_tcmd "3dTstat -mean -prefix ${workprefix}_mean_run${run}${ext} ${inputs[run]}"
done
...
log_tcmd "bet2 ${workprefix}_mean${ext} mask${ext} -f 0.3 -n -m" "mask${ext}"
...
for run in ${pruns[@]}; do
  log_echo "...run ${run}"
  log_tcmd "3dcalc -a ${inputs[run]} -b mask1${ext} -expr 'a*step(b)' -prefix ${workprefix}_bet_run${run}${ext}"
done
...
for run in ${pruns[@]}; do
  log_echo "...run ${run}"
  log_tcmd "3dcalc -a ${inputs[run]} -b mask3${ext} -expr 'a*step(b)' -prefix ${outprefix}_run${run}${ext} -datum float" 
...
for run in ${pruns[@]}; do
  log_echo "...run ${run}"
  log_tcmd "3dTcat -prefix mean_func_run${run} ${workprefix}_thresh_run${run}${ext}"
done
