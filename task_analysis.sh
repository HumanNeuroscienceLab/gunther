...
#
#     task_analysis.sh -i input1 input2 ... inputN -s label file model -s label file model --tr 1 --polort 0 -m mask -o output-directory [--oresiduals]
#  
#  This script does task analysis
#  using 3dDeconvolve
#
# 
...
cmdarg_info "header" "Script for applying warp to other images"
cmdarg_info "author" "McCarthy Lab <some address>"
## required inputs
cmdarg "i:" "inputs" "Input functionals"
cmdarg "m:" "mask" "Mask file path"
cmdarg "b:" "bgimg" "Background image"
cmdarg "o:" "output" "Output directory"
cmdarg "r:" "tr" "TR of input functionals"
## optional inputs
cmdarg "c:" "lcl" "Force local timing, see -local_times in 3dDeconvolve" false
cmdarg "g:" "global" "Force global timing, see -global_times in 3dDeconvolve" false
cmdarg "s:" "stim" "Stimulus information: label file-path model" ""
cmdarg "t:" "stim_am1" "Stimulus (amplitude-duration modulated) information: label file-path model" ""
cmdarg "u:" "stim_am2" "Stimulus (amplitude-duration modulated) information: label file-path model" ""
cmdarg "n:" "glt" "Contrast information (this is on top of any main effects of the stimulus information): label contrast" ""
cmdarg "p:" "polort" "Polort (can be -1 for nothing)" "0"
cmdarg "e:" "oresiduals" "Output residuals of task-analysis model fitting" false
cmdarg "a:" "motion" "AFNI motion parameters to include as covariates" ""
cmdarg "v:" "covars" "Additional covariate (e.g., compcor). Two arguments must be given: label filepath" ""
cmdarg "d:" "regdir" "A registration directory in the style of fsl. If given, then will transform outputs into standard space" ""
cmdarg "j:" "freedir" "Subject directory for freesurfer outputs. Should be $SUBJECTS_DIR/subject" ""
cmdarg "k:" "tostandard" "Will register to standard space if regdir is specified" false
cmdarg "x:" "tohighres" "Will register to highres space if regdir is also specified" false
cmdarg "y:" "tosurf", "Will register to native surface space if regdir and freedir is also specified" false
    
cmdarg "z:" "stopearly" "This will quit the program after saving the t-stat maps in native space. The steps of tsnr, easythresh, or normalization will not be run."
  ...
  if not regdir.nil? and not freedir.nil? and tosurf
    ...
    ["lh", "rh"].each do |hemi|
      ...
      labels.each_with_index do |label,i|
        ...
        l.cmd "mri_vol2surf --src #{statdir}/coef_#{label}#{ext} --srcreg #{regfile} --trgsubject #{freesubj} --hemi #{hemi} --surf white --out #{outdir}/reg_surf/stats/#{hemi}_coef_#{label}.mgh --sd #{freedir}"
        l.cmd "mri_vol2surf --src #{statdir}/tstat_#{label}#{ext} --srcreg #{regfile} --trgsubject #{freesubj} --hemi #{hemi} --surf white --out #{outdir}/reg_surf/stats//#{hemi}_tstat_#{label}.mgh --sd #{freedir}"
        l.cmd "mri_vol2surf --src #{statdir}/zstat_#{label}#{ext} --srcreg #{regfile} --trgsubject #{freesubj} --hemi #{hemi} --surf white --out #{outdir}/reg_surf/stats//#{hemi}_zstat_#{label}.mgh --sd #{freedir}"
      end
